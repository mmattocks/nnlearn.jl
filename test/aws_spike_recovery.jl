...
using nnlearn, BGHMM, HMMBase, Distributed, Distributions, Random, Serialization, Distributions
...
([
    ...
    ("RI", (no_sources)),
    ("EM", (no_sources))
],[.025, .025, .025, .025, .775, .075, .025, .025]),
([
    ("PS", (no_sources)),
    ("PM", (no_sources)),
    ("PSFM", (no_sources)),
    ...
    ("RI", (no_sources*10)),
    ("EM", (no_sources))
],[.05, .15, .15, .10, .05, .20, 0.0, 0.00, 0.20, 0.10, .0]),
]
job_limit=4
...
remote_machine = "18.222.227.156"
remote_pool=addprocs([(remote_machine, 1)], tunnel=true, topology=:master_worker)
librarians=[1]

@everywhere using nnlearn
    
@info "Converging ensemble..."
final_logZ = nnlearn.ns_converge!(ens, [param_set], models_to_permute, librarians, remote_pool, 0.01, backup=(true,5), model_display=no_sources)
