...
function nested_step!(e::Bayes_IPM_ensemble, perm_params::Vector{Tuple{Tuple{Int64,Vector{Float64},Distributions.Uniform{Float64}},Int64}},models_to_permute::Int64, dist_params::Tuple=(false, nothing, nothing))
    ...
    ll_contour, least_likely_idx = findmin([model.log_Li[1] for model in e.models])
...
-
    ...
    push!(e.log_Li, minimum([model.log_Li[1] for model in e.models])) #log likelihood of the least likely model - the current ensemble ll contour at Xi
...
function nested_sample_posterior_to_convergence!(e::Bayes_IPM_ensemble, permute_params, permute_limit, evidence_fraction::Float64=.001; verbose::Bool=true)
    ...
    while iterate <= N * e.Hi[end] * 10 
        ...
        warn == 1 && #"1" passed for candidates means nested_step failed to find permutation inside llh contour, if not then
            #(Lmax_model_idx = findmax([model.log_likelihood[1] for model in e.models])[2]) : #update most likely model index
...
function nested_sample_posterior_to_convergence!(e::Bayes_IPM_ensemble, permute_params::Vector{Tuple}, permute_limit::Int64, dist_params::Tuple; verbose::Bool=true)
    N = length(e.models)
    #est_std_logZ = sqrt(e.Hi[end]/N)
    #Lmax_model_idx = findmax([model.log_Li[1] for model in e.models])[2]
    ...
    while iterate < N * e.Hi[end] * 10 
        warn = nested_step!(e, permute_params, permute_limit, dist_params) #step the ensemble
        warn == 1 && #"1" passed for candidates means nested_step failed to find permutation inside llh contour, if not then
            #(Lmax_model_idx = findmax([model.log_likelihood[1] for model in e.models])[2]) : #update most likely model index
                return e #if there is a warning, just return the ensemble
        verbose && @info "Iterate: $iterate, thresh:$(N * e.Hi[end] * 10), contour: $(e.log_Li[end]), log_Xi:$(e.log_Xi[end]), log_wt:$(e.log_wi[end]) log_liwi:$(e.log_Liwi[end]), log_Z:$(e.log_Zi[end]), H:$(e.Hi[end])"
        iterate += 1
        serialize(string(e.ensemble_directory,"ensemble"),e)
    end
