...
function get_BGHMM_symbol_lh(seq::Vector{Int64}, hmm::HMM)
    ...
    lls = permutedims(lls, [2,1]) # from (K,T) to (T,K)
    log_α = permutedims(log_α, [2,1])
    log_β = permutedims(log_β, [2,1])

    log_γ = fill(-Inf, Tmaxplus1,Strand,K)
    ...
    for t in 1:size(seq)[1]-1 #iterating down the sequence
        symbol_lh = 0
        for k = 1:K #iterate over states
                state_symbol_lh = MS_HMMBase.log_prob_sum(log_γ[t,k], log(hmm.D[k].p[seq[t]])) #state symbol likelihood is the γ weight * the state symbol probability (log implementation)
                symbol_lh = logsumexp([symbol_lh,state_symbol_lh]) #sum the probabilities over states
        end
        symbol_lhs[t] = symbol_lh
    end

    return symbol_lhs[1:end-1] #remove trailing index 0
end
...
function queue_BGHMM_likelihood_calcs(observations::DataFrame, BGHMM_dict::Dict{String,Tuple{HMM, Int64, Float64}})
    ...
    BGHMM_fragments = fragment_observations_by_BGHMM(observations.PadSeq, observations.MaskMatrix, observations.PadOffset)

    lh_job_queue = RemoteChannel(()->Channel{Tuple{Tuple, Vector{BioSequence{DNAAlphabet{4}}}}}(Inf))
    job_count = 0
...
function fragment_observations_by_BGHMM(seqs::Vector{BioSequence{DNAAlphabet{4}}}, masks::Vector{Matrix{Int64}},offsets::Vector{Int64})
    likelihood_jobs = Dict{Tuple,Vector{BioSequence{DNAAlphabet{4}}}}()
...
function process_BGHMM_likelihood_queue!(lh_jobs::RemoteChannel, output_lhs::RemoteChannel, BGHMM_dict::Dict{String,Tuple{HMM, Int64, Float64}})
    ...
    while isready(lh_jobs)
        ...
        order_seq = BGHMM.get_order_n_seq(frag, order)
        coded_seq = BGHMM.code_seq(order_seq)
        
        subseq_symbol_lh::Matrix{Float64} = get_BGHMM_symbol_lh(coded_seq, partition_BGHMM)
